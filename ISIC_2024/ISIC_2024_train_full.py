...
def train_full_fold(model_name, df, sampling_rate, batch_size_train, batch_size_valid, augmentation, epochs, early_stop, use_cutmix, criterion, save, ver, output_path, device):
    new_df = pd.DataFrame()
    criterion.to(device)

    for fold in np.arange(0.0, 5.0, 1.0):
        ...
        optimizer = optim.AdamW(params=model.parameters(), lr=1e-5, weight_decay=1e-2, eps=1e-8)
        scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0 = 15, T_mult = 1, eta_min = 0)
        ...
        for epoch in range(1, epochs + 1):
            ...
            valid_loss, cm, pauc, preds = valid_one_epoch(model, valid_loader, criterion, device)
            print(f'fold {fold}, epoch {epoch}, valid_loss = {valid_loss}, pAUC = {pauc}, cofusion_matrix = {cm}')

            if valid_loss < best_loss:
                best_loss = valid_loss
                best_epoch = epoch
                predicts = pd.DataFrame(preds, columns = [f"{model_name}_predict"])
                if save:
        ...
        indices_to_concat = df[df['fold'] == fold].index
        temp_df = pd.concat([df.loc[indices_to_concat].reset_index(drop=True), predicts.reset_index(drop=True)], axis=1)
        new_df = pd.concat([new_df.reset_index(drop=True), temp_df.reset_index(drop=True)], axis = 0)
...
new_metadata = train_full_fold(model_name = "tiny_vit_5m_224",
                               df = metadata,
                               sampling_rate = 16,
                               batch_size_train = 64,
                               batch_size_valid = 64,
                               ...
                               early_stop = 5,
                               use_cutmix = True,
                               criterion = nn.BCEWithLogitsLoss(pos_weight=torch.FloatTensor([64])),
                               save = True, 
                               ver = "6", 
...
display(new_metadata)
new_metadata.to_csv(fr"Desktop\コンペ\ISIC 2024 - Skin Cancer Detection with 3D-TBP\isic-2024-challenge\train-metadata{ver}.csv", index = None)
