...
def ICIC_objective(model_name, df, folds, batch_size_train, batch_size_valid, augmentation, epochs, early_stop, use_cutmix):
    
    def objective(trial):
        sampling_rate = trial.suggest_int("sampling_rate", 10, 100, step = 10)
        trial.set_user_attr("sampling_rate", sampling_rate)

        pos_w = trial.suggest_int("pos_w", 10, 100, step = 10)
        trial.set_user_attr("pos_w", pos_w)
        ...
        for fold in folds:
            ...
            scores.append(train_one_fold(model_name, 
                                         df,
                                         fold, 
                                         sampling_rate, 
                                         batch_size_train, 
                                         ...
                                         early_stop,
                                         use_cutmix,
                                         criterion = nn.BCEWithLogitsLoss(pos_weight=torch.FloatTensor([pos_w])),
                                         save = False, 
                                         ver = "_", 
                                         output_path = "_", 
                                         device = device))
...
study.optimize(ICIC_objective(model_name = "tf_efficientnet_b0", 
                              df = metadata, 
                              folds = [0.0, 4.0], 
                              batch_size_train = 64, 
                              batch_size_valid = 64, 
