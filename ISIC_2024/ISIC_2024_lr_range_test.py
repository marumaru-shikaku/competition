...
def lr_range_test(model_name, df, fold, sampling_rate, batch_size_train, batch_size_valid, augmentation, criterion, epochs, device):
    ...
    optimizer = optim.AdamW(params=model.parameters(), lr=1e-8, weight_decay=1e-2, eps=1e-8)
    scheduler = lr_scheduler.StepLR(optimizer, step_size = 1, gamma = 5)
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_loss = train_one_epoch(model, optimizer, train_loader, criterion, device, use_cutmix = False)
...
folds = [0.0, 4.0]
for samp_rate in [1, 10, 100, 1000]:
    for weight in [1, 10, 100, 1000]:
        cr = nn.BCEWithLogitsLoss(pos_weight=torch.FloatTensor([weight]))
        for fold in folds:
            np.random.seed(seed)
            random.seed(seed)
            torch.manual_seed(seed)
            torch.cuda.manual_seed(seed)
            lr_range_test(model_name = "tf_efficientnet_b0",
                          df = metadata,
                          fold = fold,
